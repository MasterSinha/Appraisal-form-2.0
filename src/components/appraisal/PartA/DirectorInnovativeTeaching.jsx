...
export default function DirectorInnovativeTeaching({ ctx }) {
 const { faculty, docs, lectures, courseFile, projects, quals, feedback, deptActs, uniActs, society, industry, acr, journals, books, ict, research, projects2, externalProjects, patents, awards, confs, proposals, products, fdps, training, rows, sectionEmpty, emptySectionRow, getDir, setDir, getInnovDir, setInnovDir, innovativeRows, innovativeRowMax = SCORE_LIMITS.innovativeRow } = ctx;
 return (
<>
...
<tbody>
 {sectionEmpty("innovRows") ? emptySectionRow(6) : innovativeRows.map((row, index) =>{
...
<td style={TDS_DIR}><DirInput val={String(getInnovDir(index) ?? "").trim() ? clampScore(getInnovDir(index), row.max || innovativeRowMax) : ""} max={row.max || innovativeRowMax} disabled={!rowReviewable} onChange={v =>setInnovDir(index, v)} /></td>
</tr>
 );
 })}
</tbody>
